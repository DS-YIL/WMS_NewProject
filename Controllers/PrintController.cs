	...
	public class PrintController : ControllerBase
	{
		...
		public string printBarcode(PrintHistoryModel model)
		{
			...
			using (FileStream fs = new FileStream(path, fileType))
			{
				using (TextWriter tw = new StreamWriter(fs))
				{
					...
					string formateddate = model.gateentrytime.ToString("dd-MM-yyyy hh:mm:ss tt");
		...
		public string printonholdmaterials(printonholdGR onholdprintdata)
		{
			...
			try
			{
				...
				{
					//update count wms_reprinthistory table   
					reprintModel printmodel = new reprintModel();
					printmodel.inwmasterid = onholdprintdata.gateentryid;
					printmodel.inwardid = onholdprintdata.inwardid;
					printmodel.reprintedby = onholdprintdata.createdby;
					printmodel.reprintcount = 0;
					printmodel.noofprint = onholdprintdata.noofprint;
					this._poService.updatereprintstatus(printmodel);
					return "success";
				}
				else
				{
